...
from app import schemas, chatbot
...
@router.get("/sesiones")
def listar_sesiones(usuario_id: int = None, db: Session = Depends(get_db)):
...
def mensajes(sesion_id: int, db: Session = Depends(get_db)):
...
def ask(
    ...
    respuesta = chatbot.responder(db, data.mensaje)
